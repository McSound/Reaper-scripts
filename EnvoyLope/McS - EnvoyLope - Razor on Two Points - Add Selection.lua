...
function Main()
  ...
  if window == "arrange" then
    ...
    local _, razor = r.GetSetMediaTrackInfo_String(track, "P_RAZOREDITS","", false)
    local env
    ...
    if env then
      ...
      if razor ~= "" then razor = razor.." " end
      razor = razor..tostring(time1).." "..tostring(time2).." "..env_guid
      r.GetSetMediaTrackInfo_String(track, "P_RAZOREDITS", razor, true)
